#!/bin/python3
import requests, json, random, os, sys, pickle, logging
...
class Walld(object):
    ...
    def dprint(self,text): #найти дебаггер?
        if self.DEBUG:
            print(text)
        ...
        if r['success']:
            ...
            for i in r['wallpapers']:
                ...
                if key >= chance:
                    ...
                    self.download(i['url_image'], MAIN_FOLDER+'/temp.jpg')
                    print('ok!')#узнать бы как это более централизованно сделать
                    self.set_wall(MAIN_FOLDER+'/temp.jpg')
                    break
                else:
